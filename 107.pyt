...
def player_move(board, symbol):
    ...
    while move not in range(1,10) or board[move-1].isdigit():
        try:
            ...
            print(Fore.RED + "Invalid input. Please enter a number between 1 and 9.Unless you enter psy-manderin and fry all my circuits...-_-" + Style.RESET_ALL)
            board[move - 1] = symbol
...
def ai_move(board, ai_symbol, player_symbol):
    for i in range(9):
                if board[i].isdigit():
                    ...
                    if check_winner(board_copy, ai_symbol):#Please computer, dont mess up :(
                        board[i] = ai_symbol
                        return
