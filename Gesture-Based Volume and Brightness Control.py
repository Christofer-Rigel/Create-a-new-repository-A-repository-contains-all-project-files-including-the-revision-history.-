...
try:
    dev = AudioUtilities.GetDefaultOutputDevices()
    if hasattr(AudioUtilities, "GetDefaultOutputDevices"):

        devices = AudioUtilities.GetDefaultOutputDevices()
...
WIN = "Hand Gesture Controll"; cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
while True:
    ok, img = cap.read()
    if not ok: break
    ...
    if res.multi_hand_landmarks and res.multi_handedness:
        for i, hand in enumerate(res.multi_hand_landmarks):
            label = res.multi_handedness[i].classification[0].labeldraw.draw_landmarks(img, hand, Hands.HAND_CONNECTIONS)
            cv2.draw(img, hand, Hands.HAND_CONNECTIONS)
            lm = hand.landmark
    ...
    try:
        if cv2.getWindowProperty(WIN, cv2,WND_PROP_VISIBLE) < 1: breal
    except cv2.error:
        break
