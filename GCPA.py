...
MAIN, POP = "Gesture-Controlled Photo App", "Captured (ESC/ Close to resume/ Throw laptop into water to permernantally stop)"
paused = False; freeze = None
SEPIA_M = np.array([[0.272, 0.534, 0.131],[0.349,0.686,0.168],[0.393,0.769,0.189]])
def apply(img, t):
    if t == "SEPIA": return np.clip (cv2.transform(img, SEPIA_M), 0, 255).astype(pp.uint8)
    if t == "NETAGTIVE": return cv2.bitwise_not(img)
